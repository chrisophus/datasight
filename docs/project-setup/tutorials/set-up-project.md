...
This tutorial walks you through creating a datasight project for your
database — from installing the CLI to launching the web UI on your own
data. Allow about ten minutes. By the end you'll have a project directory
with a working `.env`, a database connection, and AI-generated drafts of
the schema description and example queries.
...
[Install datasight](../../use/how-to/install.md). All database
...
`time_series.yaml`
: Declare temporal structure for completeness checks. See [Declare time series](../how-to/declare-time-series.md).
...
- Writes a persistent DuckDB file (`database.duckdb` by default in the
  ...
  [Query files directly](../how-to/querying-files.md#excel-files)).
...
declarations (see [Declare time series](../how-to/declare-time-series.md)).
...
better SQL. See [Write a schema description](../how-to/schema-description.md) for guidance.

Edit `queries.yaml` with example questions and their correct SQL. See
[Create example queries](../how-to/example-queries.md) for guidance.
...
See [Configure semantic measures](../how-to/configure-measures.md) for the full `measures.yaml` workflow.
...
See [Ask questions from the CLI](../../use/how-to/ask-from-cli.md) for batch mode, export
